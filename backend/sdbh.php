...
class sdbh {
	function __construct($force_master = false){
		$this->port = 3306;
		$this -> host = 'localhost';
		$this -> dbname = 'koruposovm';
		$this -> user = 'koruposovm';
		$this -> pass = 'N2N55U*Q7WAW9WUq';
	...
	public function mselect_rows(
		...
		while(True) try{ // Добавлена проверка на выбраную строку, при пустом значении выполняется SQL-запрос без WHERE
            if (empty($select_array)) {
                $query=$this->query_exc("SELECT * FROM `$tbl_name`
